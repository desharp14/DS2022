...
template < typename G >
  class Graph {
    ...
    Graph(store_type mode, vector < G > vertex, vector < Edge > list = {}, bool type = true): store_mode {
      mode
    },
    Vertex {
      vertex
    ...
    Graph(vector < G > vertex, vector < Edge > list = {}, bool type = true): Vertex {
      vertex
    },
    Direction {
      type
    ...
    bool BELLMAN_FORD(int s, bool output = false) {
      ...
      for (int j = 0; j < N; ++j) {
        for (auto e: adj_list[j]) {
          if (d[e.v] > d[e.u] + e.w) {
            if (output) {
              cout << "Negative weighted detected." << endl;
            }
            return false;
          }
        }
